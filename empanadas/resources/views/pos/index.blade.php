...
@section('content')
{{-- ═══════════════════════════════════════════════════════════════
     POS — Layout de dos columnas: productos | carrito
═══════════════════════════════════════════════════════════════ --}}
<div class="flex h-full gap-0" x-data="posApp()" x-init="init()">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                ...
                <template x-for="producto in productosFiltrados" :key="producto.id">
                    <button @click="agregarAlCarrito(producto)"
                        ...
                        {{-- Imagen placeholder (preparado para imagen futura) --}}
    ...
    <div class="w-80 xl:w-96 flex flex-col bg-white border-l border-cream-200 shadow-xl">
        ...
        <div class="p-4 border-b border-cream-100">
            ...
            <div class="relative">
                <div class="flex items-center gap-2 p-2.5 rounded-xl bg-cream-50 border border-cream-200
                            cursor-pointer hover:border-brand-300" @click="mostrarBuscadorCliente = true">
                ...
                {{-- Dropdown buscador de cliente --}}
                <div x-show="mostrarBuscadorCliente" @click.outside="mostrarBuscadorCliente = false"
                    ...
                    <div class="p-3 border-b border-cream-100">
                        <input type="text" x-model="busquedaCliente"
                               ...
                               x-ref="inputCliente" x-init="$nextTick(() => $refs.inputCliente?.focus())">
                    </div>
                    ...
                    <div class="max-h-48 overflow-y-auto">
                        ...
                        <template x-for="c in clientesBusqueda" :key="c.id">
                            <button @click="seleccionarCliente(c)"
                                ...
                                <p class="text-xs text-gray-400" x-text="c.tipo_documento + ': ' + c.numero_documento"></p>
                            </button>
                        </template>

                        {{-- Crear cliente rápido --}}
                        <button @click="mostrarFormCliente = true; mostrarBuscadorCliente = false"
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-2">
            <template x-for="(item, idx) in carrito" :key="item.id">
                <div class="slide-in flex items-center gap-3 bg-cream-50 rounded-xl p-2.5">
                    <span class="text-xl" x-text="item.tipo_producto === 'papa_rellena' ? '🥔' : '🥟'"></span>
                    ...
                    <div class="flex items-center gap-1.5 shrink-0">
                        <button @click="decrementar(idx)"
                                class="w-7 h-7 rounded-full bg-white border border-gray-200 text-gray-600
                                       ...
                                       flex items-center justify-center font-bold text-sm transition-colors">−</button>
                        ...
                        <button @click="incrementar(idx)"
                                class="w-7 h-7 rounded-full bg-brand-500 text-white
                                       hover:bg-brand-600 flex items-center justify-center font-bold text-sm transition-colors">+</button>
                    </div>
        ...
        <div class="p-4 border-t border-cream-200 space-y-3" x-show="carrito.length > 0">
            ...
            <button @click="procesarVenta()"
                ...
                <span x-show="!procesando">💰 Cobrar <span x-text="formatPrecio(total)"></span></span>
...
<div x-show="mostrarFormCliente"
     ...
     x-transition>
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 fade-in" @click.stop>
        ...
        <form @submit.prevent="crearClienteRapido()">
            ...
            <div class="flex gap-3">
                <button type="button" @click="mostrarFormCliente = false"
                        class="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 font-medium text-sm">
                    Cancelar
                </button>
                <button type="submit"
                        class="flex-1 py-2.5 rounded-xl bg-brand-500 text-white font-bold hover:bg-brand-600 transition-colors text-sm">
                    Guardar
                </button>
            </div>
        </form>
        ...
        filtrarProductos() {
            ...
            this.productosFiltrados = this.todosLosProductos.filter(p => {
                ...
                const matchBusq = !t
                    ...
                    || p.relleno.toLowerCase().includes(t);
